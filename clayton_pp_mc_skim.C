...
TH1D *h_muPt = new TH1D("h_muPt","muon pt",NPtBins,ptMin,ptMax);
	...
	TH1D *h_muPt_ghost = new TH1D("h_muPt_ghost","ghost muon pt",NPtBins,ptMin,ptMax);
		...
		for(int jeti = 0; jeti < em->njet; jeti++){
			...
			if(em->hadronFlavor[jeti]==4){
				...
				if(em->partonFlavor[jeti]==0){
					h_jetpt_ghost->Fill(em->jetpt[jeti],em->weight);
					h_jeteta_ghost->Fill(em->jeteta[jeti],em->weight);
					h_jetphi_ghost->Fill(em->jetphi[jeti],em->weight);
				}
			}
			// muon loop
			if(em->nMu==0){continue;}
			double deltaRmin=100000.0;
			double muRelPtMin=0.0;
			double muPtMin = 0.0;
			double muPhiMin = 0.0;
			double muEtaMin = 0.0;
			for(int mui = 0; mui< em->nMu; mui++){
				...
				if(deltaRmin<0.4){
					...
					h_muPt->Fill(muPtMin,em->weight);
					h_muEta->Fill(muEtaMin,em->weight);
					h_muPhi->Fill(muPhiMin,em->weight);
					if(em->hadronFlavor[jeti]==4){
						h_muRelPt_c->Fill(muRelPtMin);	
						h_muPt_c->Fill(muPtMin,em->weight);
						h_muEta_c->Fill(muEtaMin,em->weight);
						h_muPhi_c->Fill(muPhiMin,em->weight);
					}
					else if(em->hadronFlavor[jeti]==5){
						h_muRelPt_b->Fill(muRelPtMin);	
						h_muPt_b->Fill(muPtMin,em->weight);
						h_muEta_b->Fill(muEtaMin,em->weight);
						h_muPhi_b->Fill(muPhiMin,em->weight);
					}
					else{
						if(em->partonFlavor[jeti]==1 || em->partonFlavor[jeti]==-1){
							h_muRelPt_uds->Fill(muRelPtMin);
							h_muPt_uds->Fill(muPtMin,em->weight);
							h_muEta_uds->Fill(muEtaMin,em->weight);
							h_muPhi_uds->Fill(muPhiMin,em->weight);
						}
						if(em->partonFlavor[jeti]==2 || em->partonFlavor[jeti]==-2){
							h_muRelPt_uds->Fill(muRelPtMin);
							h_muPt_uds->Fill(muPtMin,em->weight);
						...
						if(em->partonFlavor[jeti]==21){
							h_muRelPt_g->Fill(muRelPtMin);
							h_muPt_g->Fill(muPtMin,em->weight);
							h_muEta_g->Fill(muEtaMin,em->weight);
							h_muPhi_g->Fill(muPhiMin,em->weight);
						}
						if(em->partonFlavor[jeti]==0){
							h_muRelPt_ghost->Fill(muRelPtMin);
							h_muPt_ghost->Fill(muPtMin,em->weight);
							h_muEta_ghost->Fill(muEtaMin,em->weight);
							h_muPhi_ghost->Fill(muPhiMin,em->weight);
						}
					}
				}
...
auto wf = TFile::Open("/uscms/home/cmbennet/work/clayton_pp_mc_skim.root","recreate");
